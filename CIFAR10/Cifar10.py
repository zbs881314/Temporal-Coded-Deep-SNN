...
class CFAR10:
    ...
    def next_batch(self, batch_size, shuffle=False):
        if shuffle:
            ...
            xs = self.train_set[index, :]
            ys = self.train_label[index, :]
            return xs, ys
        else:
            ...
            xs = self.train_set[self.pointer:self.pointer + batch_size, :]
            ys = self.train_label[self.pointer:self.pointer + batch_size, :]
...
# for i in range(10):
#     batch_size = 10
#     xs, ys = cfar10.next_batch(batch_size, shuffle=False)
#     print(xs.shape)
#     print(ys.shape)
